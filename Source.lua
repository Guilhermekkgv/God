...
function Linux.Create(config)
    ...
    local SubtitleLabel = Linux.Instance("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 0, 1, 0),
        Position = UDim2.new(0, TitleLabel.TextBounds.X + 9, 0, 0),
        Font = Enum.Font.SourceSansBold,
    ...
    function LinuxLib.Tab(config)
        ...
        if config.Icon and config.Icon.Enabled then
            TabIcon = Linux.Instance("ImageLabel", {
                ...
                Image = config.Icon.Image or "rbxassetid://7072725342",
        ...
        function TabElements.Dropdown(config)
            ...
            local Selected = Linux.Instance("TextLabel", {
                ...
                Font = Enum.Font.Gotham,
                Text = IsMulti and (config.Default and table.concat(config.Default, ", ") or "None") or (config.Default or (config.Options and config.Options[1]) or "None"),
                TextColor3 = Linux.Theme.Text,
                TextSize = 14,
            ...
            local function UpdateDropSize()
                ...
                task.wait(0.2)
                TabContent.CanvasSize = UDim2.new(0, 0, 0, ElementContainer.AbsoluteSize.Y + DropFrame.AbsoluteSize.Y)
            end
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.Gotham,
                            Text = tostring(opt),
                            TextColor3 = (IsMulti and table.find(SelectedValue, opt)) or (not IsMulti and opt == SelectedValue) and Linux.Theme.SelectedOption or Linux.Theme.Text,
                            TextSize = 14,
            ...
            local function SetOptions(newOptions)
                ...
                SelectedValue = IsMulti and {} or (Options[1] or "None")
                UpdateSelectedText()
                PopulateOptions()
            end
