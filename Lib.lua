...
local Linux = {
    Theme = {
        ...
        Outline = Color3.fromRGB(0, 51, 102)
    }
}
    ...
    function LinuxLib.Tab(config)
        ...
        function TabElements.Dropdown(config)
            ...
            local Selected = Linux.Instance("TextLabel", {
                ...
                Font = Enum.Font.Gotham,
                Text = config.Default and (config.Multi and table.concat(config.Default, ", ") or config.Default) or (config.Options and config.Options[1]) or "None",
                TextColor3 = Linux.Theme.Text,
                TextSize = 14,
            ...
            local IsOpen = false
            local SelectedValue = config.Multi and (config.Default or {}) or (config.Default or (Options[1] or "None"))
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.Gotham,
                            Text = tostring(opt),
                            TextColor3 = (config.Multi and table.find(SelectedValue, opt)) or (opt == SelectedValue) and Linux.Theme.SelectedOption or Linux.Theme.Text,
                            TextSize = 14,
            ...
            local function SetOptions(newOptions)
                ...
                SelectedValue = config.Multi and {} or (Options[1] or "None")
                UpdateSelectedText()
                PopulateOptions()
            end
